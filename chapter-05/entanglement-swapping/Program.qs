namespace SwappingExample {
    ...
    operation Start() : Unit {
        ...
        for i in 1..4096 { 
            ...
            if (not c1 and not c3) { set res w/= 0 <- res[0]+1; }
            if (not c1 and c3) { set res w/= 1 <- res[1]+1; }
            if (c1 and not c3) { set res w/= 2 <- res[2]+1; }
            if (c1 and c3) { set res w/= 3 <- res[3]+1; }
            if (c1 == c3) { set matchingMeasurement += 1; }
        }
